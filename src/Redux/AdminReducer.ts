import { IBoolAction, IGetEntriesAction, IReducerAction, QueryEntry } from "../Types/Interfaces";
...
//AC
export const GetEntriesAC = (newEntries:QueryEntry[]) => ({type:GET_ENTRIES,entries:newEntries});
...
export const AdminReducer = (state=initialState,action:IReducerAction) => {
    switch (action.type) {
        case GET_ENTRIES:
            return {
                allEntries:(action as IGetEntriesAction).value,
        ...
        case CHANGE_ACCESS:
            return {
                ...
                itAccess:(action as IBoolAction).value,
                editedEntries:[]
            }    
        default:
            return state;
